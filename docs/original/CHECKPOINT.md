...
Đã chạy xong full-data cho lệnh trên:
...
- median mask/spec tăng `0.3373 -> 0.3503`.
- số ca mask quá nhỏ `<0.10` giảm `399 -> 261`.
- zero-box tăng `7 -> 12`.
- vẫn giữ `mask_outside_specimen = 0.0`.

**Trạng thái sau full-run:** `component_strict comp4` là bản full-data tốt nhất hiện tại cho hướng nhiều lát/nhiều u; `specimen_strict` vẫn là bản ổn định conservative đã chốt trước đó.
...
Đã chạy full-data bản conservative `component_strict comp2` ngày 2026-06-28:
...
- Chạy đủ `1393/1393` ca.
- Có `1393` mask và `1393` overlay.
- `summary.csv` có `1393` dòng data.
- `boxes.csv` có `3242` dòng data.
...
Ghi chú: `comp2` ít lem hơn nhưng bỏ sót nhiều hơn `comp4`; dùng khi ưu tiên conservative/giảm tô nhầm. `comp4` vẫn là bản slice-aware cân bằng hơn.
