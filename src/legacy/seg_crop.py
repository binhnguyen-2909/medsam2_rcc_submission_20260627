...
def focal_loss(logits, t, focal_alpha=0.25, focal_gamma=2.0):
    p = torch.sigmoid(logits.float())
    ce = F.binary_cross_entropy_with_logits(logits.float(), t.float(), reduction="none")
    pt = p * t + (1 - p) * (1 - t)
    alpha_t = focal_alpha * t + (1 - focal_alpha) * (1 - t)
...
def loss_fn(logits, t, phi, kind, alpha):
    ...
    if kind == "focaldice":
        return dice + focal_loss(logits, t)
...
def main():
    ...
    ap.add_argument("--loss", default="dicebce", choices=["dicebce", "diceboundary", "focaldice"])
