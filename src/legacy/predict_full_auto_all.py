...
from component_strict import clip_mask_to_component, decode_detections_components  # noqa: E402
...
def main():
    ...
    ap.add_argument("--specimen_strict", action="store_true")
    ap.add_argument("--min_box_spec_frac", type=float, default=0.50)
    ap.add_argument("--strict_fallback_thr", type=float, default=0.35)
    ap.add_argument("--strict_no_shrink", action="store_true")
    ap.add_argument("--component_strict", action="store_true")
    ap.add_argument("--component_max_box", type=int, default=4)
    ap.add_argument("--component_min_box_spec_frac", type=float, default=0.45)
    ap.add_argument("--component_fallback_thr", type=float, default=0.25)
    ap.add_argument("--component_min_area_frac", type=float, default=0.004)
    args = ap.parse_args()
    ...
    for i, ip in enumerate(images, 1):
        ...
        with torch.inference_mode(), ac:
            ...
            comp_ids = None
            comps = None
            if args.component_strict:
                boxes_px, comp_ids, comps = decode_detections_components(
                    obj[0].float(),
                    boxes[0].float(),
                    h,
                    w,
                    spec,
                    thr=args.thr,
                    fallback_thr=args.component_fallback_thr,
                    min_box_spec_frac=args.component_min_box_spec_frac,
                    max_box_per_component=args.component_max_box,
                    min_component_area_frac=args.component_min_area_frac,
                )
            elif args.specimen_strict:
                boxes_px = decode_detections_specimen(
                    obj[0].float(),
                    boxes[0].float(),
                    h,
                    w,
                    spec,
                    thr=args.thr,
                    fallback_thr=args.strict_fallback_thr,
                    min_box_spec_frac=args.min_box_spec_frac,
                    shrink_to_spec=not args.strict_no_shrink,
                )
            else:
            ...
            for bi, b in enumerate(boxes_px):
                ...
                if args.component_strict and comp_ids is not None and comps is not None:
                    one = clip_mask_to_component(one, comps[int(comp_ids[bi])]["mask"])
                union |= one
            if args.specimen_strict or args.component_strict:
                union = specimen_post_mask(union, spec)
        ...
        row = {
            ...
            "mask_outside_specimen": round(int((union & (spec == 0)).sum()) / max(1, mask_area), 6),
            "mode": "component_strict" if args.component_strict else ("specimen_strict" if args.specimen_strict else "baseline"),
            ...
            "component_ids_json": "" if comp_ids is None else json.dumps([int(v) for v in comp_ids], ensure_ascii=True),
        }
        ...
        for j, b in enumerate(boxes_list):
            box_rows.append({
                ...
                "component_id": "" if comp_ids is None else int(comp_ids[j]),
            })
    ...
    with open(boxes_path, "w", newline="") as f:
        fields = ["stem", "box_idx", "x0", "y0", "x1", "y1", "box_area_px", "component_id"]
